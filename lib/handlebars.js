...
function isPending(p) { 
  return (p.tdue === null || moment() < p.tdue) && (p.xfin !== 1)
}
    ...
    if (p.user == user) {
     // console.log("promise",p)
      if (isPending(p)) { pending++ }
        ...
        let ox = p.xfin ? p.xfin : 1 // optimistic xfin is 1
        let ot = p.tfin ? p.tfin : tnow // optimistic tfin is now
        let diff = moment(ot).diff(p.tdue, 'seconds')
        numerator += ox * computeCredit(diff)
        denominator += 1
      }
    }
  })
